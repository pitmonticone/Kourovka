...
- [**21.8**](Kourovka/Problem_21_8.lean): for $k \ge 4$, the horizontal class transpositions of moduli at most $k$ generate a group isomorphic to the symmetric group on $\operatorname{lcm}(2,\ldots,k)$ letters (`kourovka_21_8`).
...
- [**21.150**](Kourovka/Problem_21_150.lean): a $p$-group $G=A\rtimes B$ with $A$ and $B$ elementary abelian and $C_B(a)=1$ such that, for $H=\langle a,B\rangle$, $\operatorname{rank}(Z(H)\cap H')>\operatorname{rank}(B)$ (`kourovka_21_150`).
